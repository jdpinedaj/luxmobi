...
def insert_data_to_table(
    ...
    try:
        ...
        for i in range(
            1, 7
        ):  # This fragment of code is used to load the data from the 6 files of the traffic_counter data type
            file_suffix = (
            ...
            with open(
                ...
                if (
                    hook.get_records(
                        f"SELECT * FROM {db_name}.{schema_name}.{table_name} WHERE date = '{date}' and hour = '{hour}' and file_number = '{i}'"
                    )
                    == []
                ):
